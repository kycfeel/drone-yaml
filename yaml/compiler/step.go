...
import (
	...
	"github.com/kycfeel/drone-yaml/yaml/compiler/image"
	"github.com/kycfeel/drone-yaml/yaml/compiler/internal/rand"
)
...
func createStep(spec *engine.Spec, src *yaml.Container) *engine.Step {
	...
	for key, value := range src.Environment {
		// fix https://github.com/kycfeel/drone-yaml/issues/13
		if value == nil {
			continue
		}
		if value.Secret != "" {
